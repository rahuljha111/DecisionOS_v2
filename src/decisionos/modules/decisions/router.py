...
from decisionos.modules.decisions.schemas import (
    DecisionCreate,
    DecisionRead,
    DecisionUpdate,
)
...
async def create_decision(
    ...
    return await DecisionService(session).create_decision(
        workspace_id,
        data,
        user.id,
    )
...
async def get_decision(
    ...
    return await DecisionService(session).get_decision(
        decision_id,
        user.id,
    )
...
async def update_decision(
    ...
    return await DecisionService(session).update_decision(
        decision_id,
        data,
        user.id,
    )
...
async def delete_decision(
    ...
    await DecisionService(session).delete_decision(
        decision_id,
        user.id,
    )